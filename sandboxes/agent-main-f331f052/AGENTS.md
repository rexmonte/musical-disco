...
### Document Major Changes (#34)
When making significant config changes, model swaps, or architectural decisions, always log them in `memory/YYYY-MM-DD.md` with enough detail for future-you to understand *why*.

### End Sessions Clean (#35)
Before wrapping up significant work sessions:
1. Write a summary of what was accomplished
2. Run `git status` in workspace — commit if there are changes
3. Update memory files if needed

### Limit Directory Scope (#46)
Before editing files, confirm the working directory. Don't make changes outside the workspace unless explicitly asked. When spawning sub-agents, scope them to specific directories.

### Chat History = Cache (#47)
Never rely on conversation history as persistent storage. If something matters, write it to a file. Chat gets compacted, truncated, and lost. Files survive.
